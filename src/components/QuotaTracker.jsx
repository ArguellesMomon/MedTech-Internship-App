                  ...
                  <div key={sec.id} className={`ms-row ${isRem ? 'ms-row-rem' : ''}`}>
                    <div className="ms-row-main">
                      <div className="ms-sec-pill"
                        style={{ background: colorToSoftBg(meta.color), color: meta.color, borderColor: meta.color + '44' }}>
...
<div className="qb-overview">          {sectionSummaries.map(sec => (
            <button key={sec.id}
              className={`qb-sec-btn ${expandedSection === sec.id ? 'active' : ''}`}
              style={expandedSection === sec.id
                ? { borderColor: sec.color, background: colorToSoftBg(sec.color), boxShadow: `0 0 0 2.5px ${sec.color}44` }
...
.qb-sections-toggle {
  ...
  padding: 0; font-family: inherit; color: #aaa;
  transition: color 0.15s;
}
.qb-sections-toggle:hover { color: #ff5d8f; }
.qb-sections-toggle:hover .qb-overview-label { color: #ff5d8f; }
...
.qb-sections-wrap.open { grid-template-rows: 1fr; }
.qb-sections-wrap > .qb-overview {
  overflow: hidden;
  min-height: 0;
}
...
.qb-detail-header {
  ...
  .qb-detail-header > div { flex: 1; min-width: 120px; }
